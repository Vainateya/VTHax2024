...
export default function HeroHome() {
  ...
  const placeDummy = () => {
    ...
    setForms([
      ...updatedForms,
      { isDisabled: true, content: [{'msg': '', 'type': 'text'}], visible: false }, // Add textbox content on the left
    ]);
  };
  ...
  const addForm = (textBoxContent: any) => {
    setForms((prevForms) => {
      ...
      return [
        ...updatedForms,
        { isDisabled: false, content: { msg: '', type: 'text' }, visible: false }, // Add new form on the right
      ];
    });
  };
                ...
                >
                  ...
                  {hoveredId === block.id && (
                    <div style={{ position: 'absolute', bottom: '100%', left: '50%'}}>
                      <button type="button" className="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-3 py-2 me-2 mb-1 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800" onClick={() => handleReact(block.msg, 1)}>
                      ...
                      <button type="button" className="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-3 py-2 me-2 mb-1 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900" onClick={() => handleReact(block.msg, 0)}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="h-4 w-4">
  ...
  const handleReact = async (source: string, reward: number) => {
    try {
      const response = await fetch(
        "http://127.0.0.1:5000/react-to-source",
        {
          method: "POST",
          headers: {
